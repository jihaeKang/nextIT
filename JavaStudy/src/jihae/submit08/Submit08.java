...
//		3.아이디는 존재하지만 비밀번호가 일치하지 않으면 비밀번호가 틀렸습니다. 출력
		...
		// 입력한 id가 key값중에 존재하면 true
		// 입력한 아이디가 key값중에 존재하면 value 리턴 없으면 null
		// 입력한 아이디가 key값주엥 존재하면 null 아님 
		// if (infoMap.get(id) != null) 로 조건 써도 됨 // 아이디ㅏㄱ 맵 안에 키값으로 존재
	...
	public static ArrayList<Integer> makeLotto(){
		...
		ArrayList<Integer> mkLotto = new ArrayList<>(); 

		 while(lotto.size()<6){
		    int rand = (int)(Math.random() * 45) + 1;
		    lotto.add(rand);
		  
		} mkLotto.addAll(lotto); 
		  Collections.sort(mkLotto);
		 return mkLotto;
