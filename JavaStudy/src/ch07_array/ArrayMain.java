...
public class ArrayMain {
	...
	public static void main(String[] args) throws NoSuchAlgorithmException {
		...
		System.out.println(seoyugi.length);   // 배열은 괄호가 없어도 됨 
		...
		// 참조타입 : null
		// 기본타입: 숫자면 0 / 소수면 0.0 / boolean 이면 false
		int[] numbers = new int[3];  // 0, 0, 0 이 들어가있는 상태
		...
		// for문을 이용해서 0부터 해당 array.length 미만까지 인덱스를 순회하면 
		// 배열의 모든 요소에 접근이 가능하다.
		...
		// 코드 편집기에서 빨간줄이 쫙 그이는 에러는 컴파일 에러로,
		// 실행을 안해봐도 이클립스가 캐치를 함
		
		//아래처럼 실행했을때 발생하는 에러는 런타임 에러
//		seoyugi[4] = "크리링";
		...
		// , 로 일단 나누고 양쪽 공백을 제거해주면 된다(trim).
		 ...
		 // 해쉬(hash)
		 // 해쉬함수(암호화 알고리즘)를 이용해서 데이터를 암호화하는 기법(대표: SHA-256)
		 ...
		 // twoDir 으로부터 {7,8,9} 배열을 꺼내고 싶다.
		 int[] tempArray = twoDir[2];
		 printArray(tempArray);
		 
		 // twoDir 으로부터 6을 꺼내고싶다.
		 int six = twoDir[1][2];
		 ...
		 // 정렬
		 int[] numberArray = {12, 84, 56, 65, 34, 76};
...
//		 Arrays.sort(numberArray, Collections.reverseOrder());
		 ...
		 // 현재 12, 34, 56, 65, 76, 84
		 // 내림차순이면 84 76 65 56 34 12
