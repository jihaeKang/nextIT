...
public class NextMain {
	...
	public static void main(String[] args) {
		...
		if(changyeonExp >= 100) {
			...
			changyeonExp -= 100;  // %= 100 해서 100으로 나눈 나머지 구해줘도 됨
		}
		...
		if(minkiExp >= 100) {
			System.out.println("레벨업!!");
			minkiLevel++;
			System.out.println("민씨의 현재 레벨: " + minkiLevel);
			minkiExp -= 100;  // %= 100 해서 100으로 나눈 나머지 구해줘도 됨
		}
		...
		if(junho.exp >= 100) {
			...
			junho.exp -= 100;  // %= 100 해서 100으로 나눈 나머지 구해줘도 됨
		}
		...
		// syso 내에 객체를 넣게 되면, toString()의 실행결과가 콘솔창에 출력된다.
		System.out.println(jihye);
		System.out.println(jihye.toString()); // 둘이 똑같음
		...
		NextStudent taeyun = new NextStudent("태윤", 27, 1, 0);
		NextStudent dongyun = new NextStudent("동윤", 27, 1, 0);
		...
		// 레벨을 이용하여 정렬하기
		// Clooections.sort()는 정렬 기준을 모르면 에러가 난다.
//		Collection.sort(stuList);
		
		System.out.println("\n-============================\n");
		// 버블정렬을 이용하여 정렬
		// 변수명 한번에 바꾸기 단축키 art+shift+r
		// 레벨순으로 정렬
		// stuList [지혜][동욱][태윤],,,
