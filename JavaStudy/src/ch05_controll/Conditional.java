...
public class Conditional {
	...
	public static void main(String[] args) {
		...
		String warning = "";
		if(review.length() < 12) {
//			String warrning = "리뷰는 12자 이상 작성하세요";
		...
		if(numZero < 0) {
			...
			System.out.println("0과 같다.");  // true 값만 출력
		}else if(numZero > 0) {
			System.out.println("0보다 크다");   
//			else if(numZero > 0) {
//				System.out.println("0보다 크다"); // 엘스써도 됨 나머지니까 편한걸로
		}
		...
		if(isPowerOn == true) {
			...
			isPowerOn = false;
		}else {System.out.println("TV를 킵니다.");
		    isPowerOn = true;
			// 티비가 꺼져있는 경우
		}
		...
		if(isPowerOn == true) {
			// 티비가 켜져있는 경우
			System.out.println("TV를 끕니다.");
			isPowerOn = !isPowerOn;   // 이렇게 해도 됨
 		}else {System.out.println("TV를 킵니다.");
		    isPowerOn = true;
			// 티비가 꺼져있는 경우
		}
		
		// boolean의 true/false 값을 스위치의 on/off 처럼 사용하는
		// boolean 타입의 변수를
		...
		// 순서에 주의해야 하는 경우
		score = 92;
		if(score >= 80) {          // 여기서 이미 true 처리가 되기 때문에 밑에 조건은 보지도 않고 이걸 출력
			grade = "B";
		}else if(score >= 90) {
			grade = "A";
		}else {
		...
		int month = 3;
		 ...
		 switch(month) {
		 	...
		 	case 1:
		 		...
		 		break;
		 	case 2:
		 		System.out.println("자바끝");
		 		break;
		 	case 3:                                             //여기가 지금 트루니까 그 밑으로 다 실행해버려서 브레이크 일일 걸어줘야됨
